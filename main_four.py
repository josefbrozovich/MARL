...
for e in range(Epochs):
    ...
    for config in test_data:
        ...
        while steps < max_steps:
            ...
            # input need to be torch tensor
            next_state, reward, done1 = env.step(np.array([a1,a2,a3,a4]))
    ...
    episode_mean_test[e] = np.mean(single_episode_train)
    episode_std_test[e] = np.std(single_episode_train)
    episode_done_test[e] = np.mean(single_episode_train_done)
    episode_steps_test[e] = np.mean(single_episode_steps_train)

    print(f'Testing Episode {e + 1}, Average Reward: {episode_mean_train[e]}, STD: {episode_std_train[e]}, %goal state found: {episode_done_train[e]}, average step: {episode_steps_train[e]}')
...
DF.to_csv("results/DQN_for_MARL_15x15_more_four_agent_explore.csv", index=False)
